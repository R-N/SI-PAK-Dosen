...
class ModelAkun extends CI_Model {
    ...
    function login($username, $password){
        ...
        $sql = "
            SELECT u.id_user, u.id_pegawai, role, nama, status_user, keterangan 
            FROM user u, login_info li 
            WHERE 
                username=? 
                AND password=? 
                AND li.id_user=u.id_user
        ";
    ...
    function getUserPegawai($idPegawai){
        $sql = "
            SELECT id_user, id_pegawai, role, nama, status_user, keterangan 
            FROM user U 
            WHERE id_pegawai=?
        ";
        $query = $this->db->query($sql, array($idPegawai));
    ...
    public function getPenilaiLuar($idUser, $user=null){
        ...
        $sql = "
            ...
            FROM 
                penilai_luar pl 
                LEFT JOIN jabatan j 
                    ON pl.id_jabatan=j.id_jabatan 
                LEFT JOIN subrumpun sr 
                    ON pl.id_subrumpun=sr.id_subrumpun 
            WHERE pl.id_user=?
        ";
        $query = $this->db->query($sql, array($idUser));
        $result = $query->row();
